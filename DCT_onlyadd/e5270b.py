...
#[0.1, 0.2, ..., 3.0]
    def IVmeas_spot(self,SGDB_set=[[2,3,1,4]],none_ch=[],NP=1,Vdd=0,sweep_param=[0.1*v for v in range(0,31)],icomp=1.0e-2,vcomp=10):
        # S: GND, G: sweep, D: Vdd
        ...
        for Vg,i in zip(sweep_param,range(len(sweep_param))): #sweep_pramごとに回す
            ...
            for g in G1:#G1の要素ごとに回す（今回は１つだけなので1回だけ）
                self.SMU1.write('DV '+str(g)+',0,'+str(NP*Vg)+','+str(icomp))
                print ('DV '+str(g)+',0,'+str(NP*Vg)+','+str(icomp))#DV：一定電圧を印加
            self.SMU1.write('END')
...
#SGDBの順番注意
#TODOもーちょい分かりやすくする
    def IVmeas_sweep(self,SGDB_set=[[2,3,1,4]],none_ch=[],NP=1,vstart=3.0,vend=3.0,icomp=1.0e-3,vcomp=15,tot_step=1000):
        ...
        self.SMU1.write('WM 2, 1') #コンプライアンス電流を超えた場合に測定を終わる
