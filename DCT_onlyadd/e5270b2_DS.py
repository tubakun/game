...
#[0.1, 0.2, ..., 3.0]
    def IVmeas_spot(self,SGDB_set=[[2,3,1,4]],none_ch=[],NP=1,sweep_param=[0.1*v for v in range(0,31)],icomp=1.0e-2,vcomp=10,Vg=0):
        # S: GND, G: sweep, D: Vdd
        ...
        print ('ST 1')
        for g in G1:
            self.SMU1.write('DV '+str(g)+',0,'+str(Vg)+','+str(icomp))#0Vを印加
            print ('DV '+str(g)+',0,'+str(Vg)+','+str(icomp))
        for s in S1:
            self.SMU1.write('DV '+str(s)+',0,'+str(0)+','+str(icomp))#0Vを印加
        ...
        for Vd,i in zip(sweep_param,range(len(sweep_param))): #sweep_pramごとに回す
            print ('***** SMU1 *****')
            print ('ST {}'.format(i+2))
            self.SMU1.write('ST {}'.format(i+2))
            for d in D1:#G1の要素ごとに回す（今回は１つだけなので1回だけ）
                self.SMU1.write('DV '+str(d)+',0,'+str(NP*Vd)+','+str(icomp))
                print ('DV '+str(d)+',0,'+str(NP*Vd)+','+str(icomp))#DV：一定電圧を印加
            self.SMU1.write('END')
