...
class MEASURE():
    ...
    def TFT_IDVGS(self,ch_list=range(1,5),mparam=['I' for i in range(1,5)],
                  ar_name='p_L50W1000-1',NP=+1,mset=1,V=2.5,Vs=-0.5,Ve=2.5,step=0.1):
        ...
        for fname in os.listdir(sdir):
            if "mset_"+str(mset) in fname:
                ...
                sys.exit(self.close())
        # SMUへのプログラム書き込み
        sweep_param = [V for _ in range(400)] # 掃引パラメータ
        tot_step=100
        vstart = 1
        vend = 5
        #SGDB順番注意
        ...
        self.SMU.IVmeas_spot(SGDB_set=[[2,3,1,4]],none_ch=[],NP=1,Vdd=0,sweep_param=sweep_param,icomp=1.0e-2,vcomp=10)
        ...
        for i in range(len(sweep_param)):
            self.SMU.SMU1.write('DO '+str(i+2))
            self.SMU.SMU1.query('*OPC?')
            self.SMU.SMU1.query('*OPC?')
...
#        ar_name = 'ext-ext_L200_W200_Wb100_140' # 1 ~ 140
        ...
        mset = 3_2
        NP = 1
        measure.TFT_IDVGS(ar_name=ar_name,NP=NP,mset=mset,V=V,Vs=0,Ve=0,step=0.1)
