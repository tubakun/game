...
class MEASURE():
    ...
    def SMU_setting(self,ch=[i for i in range(1,5)],mparam=['I' for i in range(1,5)],mode='spot'):#関数の値はデフォルト値、指定がない場合これになる
        ...
        self.SMU = e5270b2.E5270_twin(GPIB0='GPIB0::9::INSTR',GPIB1='GPIB2::17::INSTR')
        # チャネルの設定
        # mparamは測定モードを指定
    ...
    def TFT_IDVGS(self,ch_list=range(1,5),mparam=['I' for i in range(1,5)],
        ...
        #一つの電圧値ごとの測定回数
        per = 10
        sweep_param_list = [value for value in [V + (0.1 * i) for i in range(0, 85)] for _ in range(per)] # 掃引パラメータ
        num = len(sweep_param_list)
        pram = [value for value in [V + (0.1 * i) for i in range(0, 85)] for _ in range(1)]
        tot_step=100
        vstart = 1
        vend = 5
        #SGDB順番注意
        ...
        self.SMU.IVmeas_spot(SGDB_set=[[2,3,1,4]],none_ch=[],NP=1,Vdd=0,sweep_param=pram,icomp=icomp,vcomp=15)
        ...
        SMU_data = self.SMU.read_buff(total_meas=num,ch=ch_list,
                                      in_sign=[[0.0 for i in range(num)],
                                               [0.0 for i in range(num)],
                                                np.array(sweep_param_list)*NP,
                                               [0.0 for i in range(num)]])
