...
class MEASURE():
    ...
    def SMU_setting(self,ch=[i for i in range(1,5)],mparam=['I' for i in range(1,5)],mode='spot'):#関数の値はデフォルト値、指定がない場合これになる """チャネルごとに変える"""
        ...
        self.SMU = e5270b2_tantai_check.E5270_twin(GPIB0='GPIB0::9::INSTR',GPIB1='GPIB2::17::INSTR')
        # チャネルの設定
        # mparamは測定モードを指定
    ...
    def TFT_IDVGS(self,ch_list=range(1,3),mparam=['I' for i in range(1,3)],
        ...
        # SMUの出力ファイルのヘッダ，パスを生
        SMU_header='time,vs,vg,is,ig,sct1,sct2\n'
        ...
        self.SMU.IVmeas_spot(SG_set=[[1,2]],none_ch=[],NP=1,Vdd=0,sweep_param=pram,icomp=icomp,vcomp=10)
...
#       ar_name = 'ext-ext_L200_W200_Wb100_140' # 1 ~ 140
        ...
        ar_name = 'CHIP1_1_1_2_check' #YXの順番
        V = 3.3
        mset = 3.3
        NP = 1
        icomp = 1.0e-2
