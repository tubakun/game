...
class MEASURE():
    ...
    def SMU_setting(self,ch=[i for i in range(1,5)],mparam=['I' for i in range(1,5)],mode='spot'):#関数の値はデフォルト値、指定がない場合これになる """チャネルごとに変える"""
        # SMUの初期設定を行う．chはSMU1の，ch2はSMU2の使用するチャネルをそれぞれ指定する．
        self.SMU = e5270b.E5270_twin(GPIB0='GPIB0::9::INSTR',GPIB1='GPIB2::17::INSTR')
        # チャネルの設定
        # mparamは測定モードを指定
    ...
    def TFT_IDVGS(self,ch_list=range(1,5),mparam=['I' for i in range(1,5)],
                  ar_name='p_L50W1000-1',NP=+1,mset=1,V=2.5, icomp=1.0e-5):
        ...
        for fname in os.listdir(sdir):
            if "mset_"+str(mset) in fname:
                ...
                sys.exit(self.close())
        # SMUへのプログラム書き込み
        sweep_param_list = [V for _ in range(600)] # 掃引パラメータ
        ...
        self.SMU.IVmeas_sweep(SGDB_set=[[2,3,1,4]],none_ch=[],NP=1,vstart=vstart,vend=vend,icomp=1.0e-6,vcomp=15, tot_step=tot_step)
        #self.SMU.IVmeas_spot(SGDB_set=[[2,3,1,4]],none_ch=[],NP=1,Vdd=0,sweep_param=pram,icomp=icomp,vcomp=10)
...
#        ar_name = 'ext-ext_L200_W200_Wb100_140' # 1 ~ 140
        ...
        NP = 1
        icomp = 1.0e-2
        measure.TFT_IDVGS(ar_name=ar_name,NP=NP,mset=mset,V=V,icomp=icomp)
